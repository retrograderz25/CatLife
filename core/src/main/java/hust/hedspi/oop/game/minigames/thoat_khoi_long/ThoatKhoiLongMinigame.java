...
public class ThoatKhoiLongMinigame implements IMinigameStrategy {
    ...
    private void shufflePuzzle() {
        // tạo bảng ban đầu xong rồi trượt ngẫu nhiên 150 lần để chắc chắn giải được
        ...
        while (moves < 150) {
            ...
            if (emptyRow > 0) validNeighbors.add(new int[]{emptyRow - 1, emptyCol}); 
            if (emptyRow < ROWS - 1) validNeighbors.add(new int[]{emptyRow + 1, emptyCol}); 
            if (emptyCol > 0) validNeighbors.add(new int[]{emptyRow, emptyCol - 1}); 
            if (emptyCol < COLS - 1) validNeighbors.add(new int[]{emptyRow, emptyCol + 1}); 
    ...
    public void render(SpriteBatch batch) {
        ...
        float frameY = (screenH - frameH) / 2f; 
        batch.draw(puzzleFrameTexture, frameX, frameY, frameW, frameH);

        
        float puzzleX = frameX + 6f; 
        float puzzleY = frameY + 6f; 
